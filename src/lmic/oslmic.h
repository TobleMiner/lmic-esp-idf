...
// Special versions
#define us2osticksCeil(us)                                                     \
    ((ostime_t)(((int64_t)(us)*OSTICKS_PER_SEC + 999999) / 1000000))
#define us2osticksRound(us)                                                    \
    ((ostime_t)(((int64_t)(us)*OSTICKS_PER_SEC + 500000) / 1000000))
#define ms2osticksCeil(ms)                                                     \
    ((ostime_t)(((int64_t)(ms)*OSTICKS_PER_SEC + 999) / 1000))
#define ms2osticksRound(ms)                                                    \
    ((ostime_t)(((int64_t)(ms)*OSTICKS_PER_SEC + 500) / 1000))
#endif
...
#else
inline u1_t table_get_u1(const u1_t *table, size_t index) {
    return table[index];
}
inline s1_t table_get_s1(const s1_t *table, size_t index) {
    return table[index];
}
inline u2_t table_get_u2(const u2_t *table, size_t index) {
    return table[index];
}
inline s2_t table_get_s2(const s2_t *table, size_t index) {
    return table[index];
}
inline u4_t table_get_u4(const u4_t *table, size_t index) {
    return table[index];
}
inline s4_t table_get_s4(const s4_t *table, size_t index) {
    return table[index];
}
inline ostime_t table_get_ostime(const ostime_t *table, size_t index) {
    return table[index];
}
