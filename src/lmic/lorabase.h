...
enum {
    ...
    MCMD_SNCH_ANS =
        0x07, // -  set new channel    : u1: 7-2=RFU, 1/0:DR/freq ACK
    // Class B
    MCMD_PING_IND =
...
enum {
    ...
    MCMD_DN2P_SET =
        0x05, // 2nd DN window param: u1:7-4:RFU/3-0:datarate, u3:freq
...
inline dr_t incDR(dr_t dr) {
    return TABLE_GET_U1(_DR2RPS_CRC, dr + 2) == ILLEGAL_RPS ? dr
                                                            : (dr_t)(dr + 1);
} // increase data rate
...
inline dr_t assertDR(dr_t dr) {
    return TABLE_GET_U1(_DR2RPS_CRC, dr + 1) == ILLEGAL_RPS ? DR_DFLTMIN : dr;
