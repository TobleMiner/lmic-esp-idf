...
static void txfsk() {
    ...
    writeReg(FSKRegPayloadLength,
             LMIC.dataLen + 1); // (insert length byte into payload))
...
// start LoRa receiver (time=LMIC.rxtime, timeout=LMIC.rxsyms,
...
static void rxfsk(u1_t rxmode) {
    ...
    // set sync config
    writeReg(FSKRegSyncConfig,
             0x12); // no auto restart, preamble 0xAA, enable, fill FIFO, 3
                    // bytes sync
    // set packet config
    writeReg(FSKRegPacketConfig1,
             0xD8); // var-length, whitening, crc, no auto-clear, no adr filter
...
#ifdef CFG_sx1276_radio
    if (v != 0x12) {
        lmic_printf("Invalid radio version number: Expected 0x12, got 0x%x. "
                    "Aborting.\n",
                    v);
#elif CFG_sx1272_radio
    if (v != 0x22) {
        lmic_printf("Invalid radio version number: Expected 0x22, got 0x%x. "
                    "Aborting.\n",
                    v);
#else
#error Missing CFG_sx1272_radio/CFG_sx1276_radio
    ...
    for (int i = 1; i < 16; i++) {
        for (int j = 0; j < 8; j++) {
            u1_t b; // wait for two non-identical subsequent least-significant
                    // bits
...
void radio_irq_handler(u1_t dio) {
    ...
    if ((readReg(RegOpMode) & OPMODE_LORA) != 0) { // LORA modem
        ...
        if (flags & IRQ_LORA_TXDONE_MASK) {
            ...
            LMIC.rssi = readReg(LORARegPktRssiValue) - 125 +
                        64; // RSSI [dBm] (-196...+63)
...
void os_radio(u1_t mode) {
    ...
    case RADIO_RX:
        ...
        startrx(RXMODE_SINGLE); // buf=LMIC.frame, time=LMIC.rxtime,
                                // timeout=LMIC.rxsyms
        break;
