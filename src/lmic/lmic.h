...
enum {
    ...
    BCN_PARTIAL = 0x01, //!< Only first (common) part could be decoded
                        //!< (info,lat,lon invalid/previous)
...
struct bcninfo_t {
    ...
    u1_t
        flags; //!< Last beacon reception and tracking states. See BCN_* values.
...
struct lmic_t {
    ...
    u4_t xchFreq[MAX_XCHANNELS]; // extra channel frequencies (if device is
                                 // behind a repeater)
    u2_t
...
#define DR_RANGE_MAP(drlo, drhi)                                               \
    (((u2_t)0xFFFF << (drlo)) & ((u2_t)0xFFFF >> (15 - (drhi))))
#if defined(CFG_eu868)
